...
These model files should be follow some specifications in order to be
“read” by mapbayr.

### 1. `$PARAM` block

From mapbayr version 1.0.0, it is not needed to define ETAs in `$PARAM`
anymore. If ETA1, ETA2 etc… are found in `$PARAM`, an error will be
returned.
...
- Strongly recommended…  
  … and **mandatory** if you have multiple types of DV, i.e. parent
  drug + metabolite:
  ...
  - Write OBS in brackets to define the observation compartment(s). Also
    ...
    to fill the ‘cmt’ column in your dataset.
  - Write ADM in brackets to define “default” administration
    ...
    function. The information is mandatory if you use
    [`adm_rows()`](https://felicienll.github.io/mapbayr/reference/adm_rows.md)
    to build your dataset in order to automatically set the value of the
    ‘cmt’ column. Especially useful if you use a model with an
    absorption from several depot compartment requiring to duplicate
    administrations lines in the data set.
...
Multiple blocks and all the options (e.g. `correlation`, `block` etc)
are allowed.
...
From mapbayr version 1.0.0, it is not needed to duplicate expressions
with ETAs in `$MAIN` anymore.
