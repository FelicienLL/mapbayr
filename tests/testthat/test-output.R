...
"
  ...
  est1 <- mapbayest(mod1, data = data1, output = "df", verbose = F)
...
test_that("mapbayests object `slots` are correct", {
  ...
  expect_named(est3$arg.ofv.id[[1]], c("iddata", "idDV", "idcmt"))
  expect_named(est3$arg.ofv.id[[2]], c("iddata", "idDV", "idcmt"))
